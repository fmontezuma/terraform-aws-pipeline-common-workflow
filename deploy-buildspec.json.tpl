...
phases:
  ...
  build:
    commands:
      - docker run --rm -v $(pwd):/apps -v ~/.kube/config:/root/.kube/config alpine/helm:2.9.0 template ./helm-microservice -f values/${project_name}-${microservice_name}/common.yml -f values/${project_name}-${microservice_name}/$ENV.yml --set image.tag=$TAG > ${project_name}-${microservice_name}.yml
