resource "aws_codebuild_project" "deploy" {
  ...
  source {
    ...
    git_clone_depth = 1
    buildspec = templatefile("${path.module}/deploy-buildspec.json.tpl", { project_name = "${var.project_name}", microservice_name = "${var.microservice_name}", env_val = "${var.environments[count.index]}", region = "${data.aws_region.current.name}", only_deploy = var.only_deploy, account_id = var.account_ids[count.index] })
  }
}
